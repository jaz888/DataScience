...
#                 old_betas[:] = new_betas[:]
#                 for i in range(sample_size):
#                     #print old_betas[0]+old_betas[1]*X[i][1]+old_betas[2]*X[i][2]+old_betas[3]*X[i][3]
...
for topic_id in ageY_genderX1_topicX2_coefficients:
    if ageY_genderX1_topicX2_coefficients[topic_id][2] != 0.0:
        top_beta.append([topic_id, ageY_genderX1_topicX2_coefficients[topic_id][2]])
        #print topic_id, ageY_genderX1_topicX2_coefficients[topic_id][2]

top_beta = sorted(top_beta, key=lambda x: x[1])
for item in top_beta:
    print item
# print top_beta
...
q4_res = {}
for industry in industries:
    if industries[industry] > 30 and industry != 'indUnk':
        Y = np.arange(sample_size, dtype=np.float).reshape(sample_size, 1)
        Y_row_id = 0
        for user_id in user_topic_prob:
...
for industry in q4_res:
    ...
    for i in range(20):
        print industry, q4_res[industry][i][0],q4_res[industry][i][1],q4_res[industry][i][2],q4_res[industry][i][3],q4_res[industry][i][3]
